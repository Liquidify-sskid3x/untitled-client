...
public class Panel {
    ...
    public static int dynamicAccent = 0xFF0a84ff;
    ...
    private static final int COLOR_HOVER   = 0x220a84ff; // this one is intentionally transparent ok...?
    ...
    public void render(GuiGraphics g, int mouseX, int mouseY) {
        ...
        if (listeningKeybind != null) {
            ...
            if (bannedMessage != null
                    && System.currentTimeMillis() - bannedMessageTime > 2000) {
                bannedMessage = null;
            }
        }
            ...
            for (int key = 0; key < GLFW.GLFW_KEY_LAST; key++) {
                ...
                if (down && !prevKeys[key]) {
                    if (key == GLFW.GLFW_KEY_ESCAPE || key == GLFW.GLFW_KEY_ENTER
                        ...
                        if (key == GLFW.GLFW_KEY_MINUS && boxBuffer.isEmpty())
                            boxBuffer = "-";
                    }
                }
                prevKeys[key] = down;
        ...
        for (Module module : modules) {
            ...
            int textColor = module.isEnabled() ? dynamicAccent : 0xFFAAAAAA;
        ...
        g.renderOutline(x, y, WIDTH, mY - y, dynamicAccent);
    ...
    private int renderSetting(GuiGraphics g, net.minecraft.client.gui.Font font,
        ...
        if (setting instanceof FloatSetting fs) {
            ...
            g.fill(barX, barY, barX + (int)(SLIDER_W * pct), barY + 4, dynamicAccent);
            ...
            g.renderOutline(boxX, boxY, BOX_W, 10, active ? dynamicAccent : 0xFF444466);
            ...
            g.renderOutline(checkX, checkY, 10, 10, dynamicAccent);
            if (bs.getValue())
                g.fill(checkX + 2, checkY + 2, checkX + 8, checkY + 8, dynamicAccent);
            mY += SETTING_H;
            ...
            if (valW + font.width(setting.getName()) + PADDING * 3 > WIDTH) {
                g.drawString(font, val, x + PADDING, mY + 13, dynamicAccent, false);
            } else {
                g.drawString(font, val, x + WIDTH - valW - PADDING, mY + 4,
                        dynamicAccent, false);
            }
            mY += SETTING_H;
            ...
            if (expandedModule == module) {
                for (Setting<?> setting : module.getSettings()) {
                    if (setting instanceof FloatSetting fs) {
                        ...
                        int previewX = x + WIDTH - 16;
                        int previewY = mY + 5;
                        if (isHovered(mouseX, mouseY, previewX, previewY, 12, 12)) {
                            Minecraft.getInstance().setScreen(
                                    new ColorPickerScreen(
                                            Minecraft.getInstance().screen, cs));
                            return true;
                        }
                        mY += SETTING_H;

                    } else if (setting instanceof KeybindSetting ks) {
                        if (isHovered(mouseX, mouseY, x + 2, mY + 2,
                                WIDTH - 4, SETTING_H - 4)) {
                            listeningKeybind = ks;
                            ks.setBinding(true);
                            bannedMessage = null;
